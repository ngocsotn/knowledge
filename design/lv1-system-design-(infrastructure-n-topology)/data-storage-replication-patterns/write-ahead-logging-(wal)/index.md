...
### Q1: What is log sequence number (LSN) tracking in WAL-based replication?
- **Answer:** LSN is a 64-bit monotonically increasing integer representing the exact byte offset of a record inside the WAL log. During replication, lagging replica nodes report their current LSN to the master. The master compares this with its latest LSN and streams only the missing byte-level WAL records starting from that exact offset, guaranteeing precise, incremental log catch-up.
