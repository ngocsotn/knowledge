# Redis Caching Access Patterns

Deep-dive covering Cache-Aside, Read-Through, Write-Through, and Write-Behind patterns.
...
### Q1: What is Write-Behind (Write-Back) Caching, and what are its risks?
- **Answer:** Under Write-Behind caching, the application writes data strictly to the cache layer first. An asynchronous worker or background thread periodically flushes these changes to the primary database.
- **Risks:** High risk of data loss. If the cache system (Redis) crashes before the background flusher writes changes to the primary database, the data is lost permanently.
